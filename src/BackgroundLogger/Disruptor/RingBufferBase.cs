...
abstract class RingBufferBase<T>
{
...
#pragma warning restore CS0169 // Field is never used
  ...
  protected MultiProducerSequencer _sequencer;
...
#pragma warning restore CS0169 // Field is never used
  ...
  protected RingBufferBase(MultiProducerSequencer sequencer, int bufferPad)
  {
    _sequencer = sequencer;
    _bufferSize = sequencer.BufferSize;

    if (_bufferSize < 1) throw new ArgumentException("bufferSize must not be less than 1");

    if (!_bufferSize.IsPowerOf2()) throw new ArgumentException("bufferSize must be a power of 2");

#if NET6_0_OR_GREATER
    _entries = GC.AllocateArray<T>(_bufferSize + 2 * bufferPad, true); // new T[_bufferSize + 2 * bufferPad];
#else
  ...
  ///   will depend on the type of <see cref="MultiProducerSequencer" /> that is being used.
  /// </summary>
  ...
  ///   </para>
  ///   <para>
  ///     If there is not enough space available in the ring buffer, this method will block and spin-wait using
  ///     <see cref="AggressiveSpinWait" />, which can generate high CPU usage.
  ...
  ///     If there is not enough space available in the ring buffer, this method will block and spin-wait using
  ///     <see cref="AggressiveSpinWait" />, which can generate high CPU usage.
  ///     Consider using <see cref="TryNext(int, out long)" /> with your own waiting policy if you need to change this
  ///     behavior.
  ///   </para>
  ///   <para>
  ///     Example:
  ...
  {
    if (n < 1 || n > _bufferSize) ThrowHelper.ThrowArgMustBeGreaterThanZeroAndLessThanBufferSize();
  ...
  public void Signal() => _sequencer.GetWaitStrategy().SignalAllWhenBlocking();
